...
// physical constants
const utype PI = 3.14159265359;
const utype HCONST = 6.62606957e-27;
const utype CSPEED = 29979245800;
const utype KBOLTZMANN = 1.3806488e-16;
const utype STEFANBOLTZMANN = 5.6704e-5;
...
__global__ void plancktable(
	...
	if (x < nwave && t < 802) {
		...
		// building temperature grid from 1 K to 7999 K at 2 K resolution
		// and then from 8000 K to approx. 80'000 K at 20 K resolution
		// and Tstar and Tint
		if(t < 800){
			if(p_iter < 5){
				T = (t + p_iter * 800) * 2 + 1;
		...
		planck_grid[x + (t + p_iter * 800) * nwave] = 0.0;
		...
		if(T > 0.01){
			...
			for(int n=1;n<100;n++){
				planck_grid[x + (t + p_iter * 800) * nwave] += D * analyt_planck(n, y_bot, y_top);
			}
		}

		planck_grid[x + (t + p_iter * 800) * nwave] /= deltalambda[x];
	}
}
...
__global__ void plancktable_sampling(
	...
	if (x < nwave && t < 802) {
		...
		// building temperature grid from 1 K to 7999 K at 2 K resolution
		// and then from 8001 to 87981 at 20 K resolution
		if(t < 800){
			if(p_iter < 5){
				T = (t + p_iter * 800) * 2 + 1;
			}
		...
		// including stellar and interior temperatures
		if(p_iter == 9){
			if(t == 800){
				T = Tstar;
			}
		...
		if(T > 0.1){
			planck_grid[x + (t + p_iter * 800) * nwave] = planck_func(lambda[x], T);
		}
	}
}
	...
	if (x < nwave){
		...
		if(realstar == 1){
			...
			for (int xl = 0; xl < nwave; xl++){
				...
				num_flux += deltalambda[xl] * PI * planck_grid[xl + 8000 * nwave];
		...
		if(x==0) printf("\nEnergy budget corrected by a factor of %.2f percent.\n", 100.0 * (corr_factor - 1.0));
		...
		if(realstar == 1){
			...
			planck_grid[x + 8000 * nwave] *= corr_factor;
...
__global__ void kappa_interpol(
    ...
    if(i < nlay_or_nint){
        ...
        if(kappa_kernel_value < 0.001){
            ...
            t = min(entr_ntemp-1.0, max(0.0, t));
            ...
            p = min(entr_npress-1.0, max(0.0, p));
...
__global__ void planck_interpol_layer(
	...
	if (x < nwave){
		...
		if(i < numlayers){
			...
			utype t;
			if(tempcell[i] <= 8001){
				t = (tempcell[i] - 1.0) / 2.0;
			}
			if(tempcell[i] > 8001){
				t = (tempcell[i] - 8001.0) / 20.0 + 4000.0;
			}
			if (t < 0) {
				planckband_cell[i + x * (numlayers + 2)] = 0.0;
			}
		...
		if (i == numlayers){
			if(realstar==1){
				...
				planckband_cell[i + x * (numlayers + 2)] = planck_grid[x + 8000 * nwave];
			}
		}
		if (i == numlayers+1){
			planckband_cell[i + x * (numlayers + 2)] = planck_grid[x + 8001 * nwave];
		}
	}
}
...
__global__ void fband_iso_notabu(
	...
	if (x < nbin && y < ny) {
		...
		for (int i = 0; i < numinterfaces; i++){
			...
			if (i == 0){
				// for singlewalk == 1 there is no downward flux contribution to the upward stream
				utype reflected_part = F_dir_wg[y+ny*x+ny*nbin*i] + F_down_wg[y+ny*x+ny*nbin* i];
				
				utype internal_part = PI * planckband_lay[numinterfaces + x * (numinterfaces-1+2)];
				
				F_up_wg[y+ny*x+ny*nbin* i] = (1.0 - singlewalk) * reflected_part + internal_part;
				//F_up_wg[y+ny*x+ny*nbin* i] = reflected_part + internal_part; // for testing reasons
			}
			else {
...
//				// direct stellar beam
//				Fc_dir_wg[y+ny*x+ny*nbin*i] = F_dir_wg[y+ny*x+ny*nbin*(i+1)] * exp(del_tau_up / mu_star);
...
//				// direct stellar beam
//				F_dir_wg[y+ny*x+ny*nbin*i] = Fc_dir_wg[y+ny*x+ny*nbin*i] * exp(del_tau_low / mu_star);
		...
		for (int i = 0; i < numinterfaces; i++){
			...
			if (i == 0) {
				// for singlewalk == 1 there is no downward flux contribution to the upward stream
				utype reflected_part = F_down_wg[y+ny*x+ny*nbin* i] + F_dir_wg[y + ny * x + ny * nbin * i];
...
__global__ void rad_temp_iter(
	...
	if(i < numlayers){
		...
		utype combined_F_net =  F_net_diff[i] + F_temp;
		...
		if(varydelta == 1){
			...
			if(combined_F_net != 0){
				delta_t = deltat_prefactor[i] * play[i] / pow(abs(combined_F_net), 0.9); // through tweaking 0.9 found to be most stable
			}
		}
		
		utype delta_T = combined_F_net / (pint[i] - pint[i+1]) * delta_t;
		
		// limits large temperature jumps for increased stability
		if(abs(delta_T) > 500.0){
			delta_T = 500.0 * combined_F_net/abs(combined_F_net);
		}
		...
		tlay[i] = min(max(tlay[i],1.0), 2e4); // temperature limited to max. 2e4 K, to decrease the runtime.
		
		// abort conditions
		// local radiative equilibrium
		bool condition1 = abs(combined_F_net)/(STEFANBOLTZMANN*(tlay[i]*tlay[i]*tlay[i]*tlay[i])) < local_limit;
		// to prevent too high temperatures in the deep (i.e. radiative equilibrium is not necessary there)
		bool condition2 = tlay[i] > 1.5e4 && play[i] > 1e6;
...
__global__ void conv_temp_iter(
	...
	if(i < numlayers){
		...
		if(combined_F_net != 0){
			delta_t = deltat_prefactor[i] * t_rad / pow(abs(combined_F_net), 0.5); // 0.5 found to be most stable for the radiative-convective interplay
		}
		
		utype delta_T = g / c_p_lay[i] * combined_F_net / (pint[i] - pint[i+1]) * delta_t;
		
		// limits large temperature jumps for increased stability
		if(abs(delta_T) > 20.0){
			delta_T = 20.0 * combined_F_net/abs(combined_F_net);
		}
